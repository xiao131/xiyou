...
const CardComponent: React.FC<CardProps> = ({ card, onClick, disabled, playable }) => {
  ...
  return (
    ...
    >
       {/* Card Top Header */}
       <div className="relative h-8 flex items-center justify-between px-2 bg-stone-950/80 border-b border-white/10 z-20">
            <span className={`text-xs font-bold tracking-widest h-font text-stone-300`}>
            ...
            <div className={`
                ...
                border-2 border-stone-900 shadow-lg -mr-4 -mt-4 z-30
            `}>
      ...
      {/* Card Image Area */}
      <div className="relative h-36 w-full bg-black overflow-hidden border-y border-stone-800 group-hover:h-40 transition-all duration-300">
        <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110" 
             style={{ backgroundImage: `url('${bgImage}')` }}>
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
        {card.rarity === 'BOSS' && <div className="absolute top-1 left-1 text-red-500 bg-black/50 rounded-full p-1"><Skull size={14}/></div>}
      </div>

      {/* Card Name Ribbon */}
      <div className="relative -mt-4 mx-1 py-1 bg-stone-900/95 border-y border-amber-800/60 shadow-md text-center z-10">
        <div className="text-sm font-bold text-amber-100 h-font tracking-widest text-shadow-sm truncate px-1">
            {card.name}
        </div>
      </div>

      {/* Card Description */}
      <div className="flex-grow p-2 flex flex-col items-center justify-start pt-4 relative bg-black/20">
        <div className="absolute inset-0 texture-paper opacity-10 pointer-events-none"></div>
        <p className="text-xs text-stone-200 font-serif leading-snug text-center relative z-10 drop-shadow-md">
          {card.description}
        </p>
        
        {card.exhaust && (
            <div className="mt-auto mb-1 text-[10px] font-bold text-red-400 border border-red-900/50 px-2 py-0.5 rounded bg-black/60">
                消耗
            </div>
        )}
      </div>
